...
def hack(textc: str, tslov: Dict[str, float]) -> List[str]:
    pt = ''
    for i in range(26):
        pt = decrypt(textc, i)
    ...
    print(sum)
    max_keys = sorted(sum, key=sum.get, reverse=False)[:2]
    for m in max_keys:
        print(m)
        print("удалить")
    all_pop = []
    for k in max_keys:
        all_pop.append(slov.get(k))
    return all_pop


def sumk(frequency_text: Dict[str, float], frequency_cipher: Dict[str, float])\
                                                                      -> float:
    sumh = 0
